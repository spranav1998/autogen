...
def text_preprocess(text, remove_html=False):
        text = text.replace("```html\n", "").replace("<body>", "").replace("</body>\n```", "").replace("\n", "")
        text = text.replace("\n\n", "").replace("```", "").replace("<!DOCTYPE html>", "")
...
class ConversableAgent(Agent):
    ...
    def generate_oai_reply(
        ...
        self.llm_config[
            "stream"] = True if sender else False  # do not send message if the sender is None, that is it's an internal system message
        sender_name = self.name
        self.llm_config["chunk_callback"] = lambda event, message: self.send_message_to_socket(event, sender_name,
                                                                                               text_preprocess(str(message)))
        self.llm_config["sid"] = self.sid
        response = client.create(
            self.llm_config,
            context=messages[-1].pop("context", None), messages=self._oai_system_message + messages
        )
        response_text = client.extract_text_or_function_call(response)[0]
        # print(response_text)
        # print(type(response_text))
        if self.socket:
            self.send_message_to_socket("prompt", str(self.name) , text_preprocess(str(response_text)))
    ...
    def check_termination_and_human_reply(
        ...
        if self.human_input_mode == "ALWAYS":
            reply = self.get_human_input(
                f"Provide feedback to {sender.name}. or type 'exit' to end the conversation with this bot"
            , f'{sender.name}')
            ...
            if self._consecutive_auto_reply_counter[sender] >= self._max_consecutive_auto_reply_dict[sender]:
                if self.human_input_mode == "NEVER":
                    ...
                    reply = self.get_human_input(
                        f"Provide feedback to {sender.name}. or type 'exit' to end the conversation with this bot"
                        if terminate
                        else f"Provide feedback to {sender.name}. or type 'exit' to end the conversation with this bot"
                    , f'{sender.name}')
                    no_human_input_msg = "NO HUMAN INPUT RECEIVED." if not reply else ""
                    # if the human input is empty, and the message is a termination message, then we will terminate the conversation
        ...
        if reply == "exit":
            ...
            self._consecutive_auto_reply_counter[sender] = 0
            if self.socket:
                self.send_message_to_socket('prompt', str(self.name), text_preprocess(reply))
            return True, None
    ...
    def get_human_input(self, prompt: str, sender_name: str) -> str:
        """Get human input.
        ...
        if self.socket:
            ...
            self.send_message_to_socket("prompt", str(sender_name), text_preprocess(prompt))
            @self.socket.on('user_input')
            def user_input_event(data):
                nonlocal reply
                data = json.loads(data)
                bot_name = data.get('bot_name')
                if str(sender_name) == bot_name:
                    reply = data.get('feedback')
                    self.send_message_to_socket("feedback", str(sender_name), text_preprocess(prompt))
                # self.send_message_to_socket("prompt", "User", reply)
            
            wait_timer = 0
            while reply is None:
                self.socket.sleep(1)
                wait_timer += 1
                if wait_timer >= 300: # 5 Mins
                    reply='exit'
                    break
        else:
            reply = input(prompt)
